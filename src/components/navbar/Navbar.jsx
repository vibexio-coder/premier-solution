...
const Navbar = () => {
  ...
  // ✅ Set underline on active route (on load + route change)
  useEffect(() => {
    const activeItem = navRef.current?.querySelector(".active-link");
    if (activeItem) moveLine(activeItem);
  }, [location.pathname]);
    ...
    <div className="bg-[#1C4746] text-white shadow-[0px_4px_6px_-2px_rgba(237,237,237,0.15)] relative z-50">
      ...
      <div className="hidden md:flex justify-between items-center poppins font-semibold text-[14px] leading-[14px] tracking-[0.05em] pt-5 pb-2 px-6 sm:px-10 lg:px-16 xl:px-[90px]">
        ...
        <div className="flex items-center gap-[29px]">
          <button className="flex items-center text-[12px] gap-3">
            <PhoneIcon />
            7708566732
          </button>

          <button className="flex items-center text-[12px] bg-white text-[#1c4746] rounded-[20px] py-[7px] px-5 gap-3">
            <WhatsAppIcon />
            Whatsapp
          </button>
        </div>
